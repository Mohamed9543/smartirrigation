...
function LoginForm({ t, isRTL, onOpenSignup }) {
  ...
  const handleLogin = async () => {
    ...
    const em = normalizeEmail(email),
      pw = String(password || "");
    if (!em || !pw) {
      setFieldError(t("login.fillFields"));
      return;
    }
    if (!EMAIL_REGEX.test(em)) {
      setFieldError(t("login.invalidEmail"));
      return;
    }
    setLoading(true);
    try {
...
function SignupForm({ t, isRTL }) {
  ...
  const handleSignup = async () => {
    ...
    if (fn.length < 2) {
      setFieldError(t("signup.firstNameTooShort"));
      return;
    }
    if (ln.length < 2) {
      setFieldError(t("signup.lastNameTooShort"));
      return;
    }
    if (ad.length < 5) {
      setFieldError(t("signup.addressTooShort"));
      return;
    }
    if (!EMAIL_REGEX.test(em)) {
      setFieldError(t("signup.invalidEmail"));
      return;
    }
    if (pw.length < 8) {
      setFieldError(t("signup.passwordTooShort"));
      return;
    }
    setLoading(true);
    try {
...
function FieldInput({
  ...
  return (
    <View style={[s.inputWrap, extraStyle]}>
      {icon && (
        <View
          style={[s.inputIconWrap, { [isRTL ? "right" : "left"]: 12 }]}
          pointerEvents="none"
        >
