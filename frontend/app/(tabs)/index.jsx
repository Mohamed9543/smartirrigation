...
export default function HomeScreen() {
  ...
  // MODIFIED: Calcul des besoins UNIQUEMENT en mm
  const calculateWaterNeeds = (culture) => {
    ...
    const debitLH      = culture.irrigation?.debit || 1000;
    
    // Calcul du débit en mm/h
    const debitMmh = debitLH / surface;
    
    // Eau à apporter en mm = ETc / efficacité (par défaut 90% pour goutte-à-goutte)
    // Valeur simplifiée pour l'affichage sur la home
    const eauMm = etc / 0.9;
    
    // Temps d'irrigation en minutes
    const tempsHeures = eauMm / debitMmh;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F4F6F8' }}>
      ...
      >
        ...
        {/* Cultures - MODIFIED: Affichage en mm uniquement */}
              ...
              {cultures.slice(0, 3).map((culture) => {
                ...
                return (
                  <View key={culture._id} style={{ backgroundColor: '#fff', padding: 16, borderRadius: 12, marginBottom: 8, borderWidth: 1, borderColor: '#f3f4f6' }}>
                    ...
                    {/* MODIFIED: Affichage en mm au lieu de litres */}
                    <Text style={{ fontSize: 26, fontWeight: '700', color: '#16a34a', marginBottom: 4 }}>{needs.eauMm} mm</Text>
                    <Text style={{ fontSize: 11, color: '#9ca3af', marginBottom: 6 }}>
                      ETc={needs.etc} mm · Kc={culture.kcActuel || 0.65} · Débit: {needs.debitMmh} mm/h
                    </Text>
